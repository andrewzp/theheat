...
**Status:** ✅ **SHIPPED** in [#215](https://github.com/andrewzp/theheat/pull/215) (0.9.22.0, squash-merged `3800f84`). Implemented as specced, plus two hardenings from the Codex *diff-stage* review: `max_by_key`/`reduce_by_key`/`_merge_data_source_failures` iterate `sorted()` keys (deterministic merged-dict bytes), and the dev-time equivalence harness became a committed golden fixture (`tests/fixtures/merge_state_golden.json`, regenerated by `scripts/gen_merge_golden.py`).
