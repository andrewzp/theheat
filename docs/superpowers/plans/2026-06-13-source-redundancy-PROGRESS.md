...
| R-05 | river_gauges (403s) | Open-Meteo Flood (GloFAS, model-framed) | independent(model) | A | R-00 | TODO (groundwork done) | | DEFERRED to a fresh tranche: endpoints+p75 live-verified, USGS gauge coords gathered — see `docs/superpowers/specs/2026-06-14-r05-r07-groundwork.md`. Open: discharge↔ft impedance + p75 semantics. |
| R-06 | firms (product gaps) | VIIRS_SNPP→NOAA20→NOAA21→MODIS chain | same-provider | A | R-00 | DONE | #PENDING | product chain in fetch_fires primary; non-first product = source_leg, NO grade; all-empty→[] (no HMS); all-fail→HMS |
| R-07 | coral_dhw (NOAA 403s) | CRW ERDDAP `noaacrwdhwDaily` grid | same-provider | A | R-00 | TODO (groundwork done) | | DEFERRED: ERDDAP host/var/format live-verified (host is `coastwatch.noaa.gov`, NOT the plan's `pfeg` — that 404s). Open: hardcoded station→grid coord map. See groundwork doc. |
