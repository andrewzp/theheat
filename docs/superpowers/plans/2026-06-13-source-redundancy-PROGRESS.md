...
| R-07 | coral_dhw (NOAA 403s) | CRW ERDDAP `noaacrwdhwDaily` grid | same-provider | A | R-00 | DONE | | Endpoint recovered on fresh probe; ERDDAP point witness samples pinned CRW station coords, adds observed_alt_host grade, and records degraded telemetry `served via crw_erddap`. |
