...
| R-04 | gdacs (EU 9/40) | ReliefWeb (UN OCHA) | independent | A | R-00 | AWAITING-ANDREW(appname-approval-email) | | ReliefWeb appname request submitted 2026-06-15 for `TheHeat-GDACSBackup-RW7K2Q`; corrected follow-up submitted with Andrew's working email after `contact@theheat.app` proved unusable. Immediate API probe still returns 403 until ReliefWeb approves it. |
...
- **R-04 ReliefWeb: waiting on appname approval email.** ReliefWeb's API now returns
  ...
  free appname request on 2026-06-15 for `TheHeat-GDACSBackup-RW7K2Q`. The first request used
  `contact@theheat.app`, but Andrew confirmed that inbox does not work; Codex submitted a corrected
  follow-up request the same day with Andrew's working email and an explicit note to ignore/replace
  the dead contact address. ReliefWeb's form says they review and respond within two business days,
  but also warns that non-organization emails may not be processed. Once approved, provide/set the
  approved value as `RELIEFWEB_APPNAME`, then R-04 can be built + verified. The lane's STOP rule
...
| 2026-06-15 | Codex | R-04(appname-requested) | Submitted ReliefWeb appname request for `TheHeat-GDACSBackup-RW7K2Q`; after Andrew flagged `contact@theheat.app` as unusable, submitted a corrected follow-up with Andrew's working email. Google Forms confirmation returned both times, and an immediate API probe still returned 403 pending approval. |
