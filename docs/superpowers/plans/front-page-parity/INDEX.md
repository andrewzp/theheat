...
| 414 | [row-414-heat-records-cluster.md](/Users/andrewpuschel/Documents/Claude/theheat/docs/superpowers/plans/front-page-parity/row-414-heat-records-cluster.md) | **PR-A MERGED (#422); PR-B built UNMERGED + design PIVOT 2026-07-08 eve.** PR-A = honest namer + clustering (3 codex rounds, on `main`). PR-B daily-only mechanism on `origin/feat/records-cluster-detection` @ `ccc99f7` — codex proved it's **US-only in prod + marginal**; **NOT merge-ready.** A codex reasoning stress-test **killed the reganom-fusion idea** (implied-attribution; reganom is an *effect* sensor + cadence-lagged) and confirmed the direction: **tier-aware STANDALONE** (daily+monthly+all-time members, gated on record significance, no fusion) — that also makes it global. Canonical: [docs/handoffs/2026-07-08-records-cluster.md](/Users/andrewpuschel/Documents/Claude/theheat/docs/handoffs/2026-07-08-records-cluster.md). Flag OFF + manual-approval. | tier-aware rework |
