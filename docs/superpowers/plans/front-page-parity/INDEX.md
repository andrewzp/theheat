...
| 414 | [row-414-heat-records-cluster.md](/Users/andrewpuschel/Documents/Claude/theheat/docs/superpowers/plans/front-page-parity/row-414-heat-records-cluster.md) | **✅ SHIPPED 2026-07-08 eve — tier-aware standalone MERGED.** PR-A namer+clustering (#422) + mechanism (#424, `988312b`, v0.9.99.0) + PR-C writer voice/fact-check/dryrun-harness (#425, `54399d3`). **tier-aware STANDALONE** (one member/city at strongest high tier all_time>monthly>daily; `is_significant_cluster`=≥1 all_time OR ≥3 monthly; global via world monthly/all-time; **no reganom fusion, no cause attribution**; tier-agnostic `place_key` dedup; suppress only daily drafts; `records_provenance` tense honesty). codex-xhigh APPROVED (mechanism 2 rounds — r1 caught world-monthly US-only trap + dedup + daily-leak + provenance, all fixed; PR-C 2 rounds). Flag `THEHEAT_RECORDS_CLUSTER_ENABLED` **OFF** + manual-approval. **Voice realigned to the house signature move — #429 MERGED 2026-07-10** (`3cc28a8`, v0.9.100.0; new move 4 = system clause is the SHIFT not the cause, house ✗/✓ example, docstring editing contract, fact-check rule r) unchanged; codex-xhigh clean APPROVE r1; Andrew approved the before/after). Then dryrun/flag-flip = Andrew's call. Canonical: [docs/handoffs/2026-07-09-next-session.md](/Users/andrewpuschel/Documents/Claude/theheat/docs/handoffs/2026-07-09-next-session.md). | dryrun sample + flag flip → Andrew |
