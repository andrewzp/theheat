...
| | |
|---|---|
| Bot commit | `0.9.108.1` per `main`'s `VERSION` file (unchanged since checked 2026-07-16). Rebase onto fresh `main` conflicted again this session — identical conflict every attempt since Jul 17 (`main` has advanced past `#449` with unrelated economics/ops commits [#385, #449, #466] that also touch `docs/IMPROVEMENT_PLAN.md`); rebase aborted per the runbook fallback, `daily-plan-current` continues unchanged from its Jul 31 tip. No `main` commit has landed at all since `4b4d965` (2026-07-24) — now 8 days quiet, consistent with the billing outage below. |
...
| Last cycle A-rate | **— (0 fresh drafts, Aug 1 — 9th consecutive zero-fresh cycle, dry spell now a full 9 days).** Most recent measured: **50% (2/4, Jul 23) — bar NOT cleared** (not a majority; same convention as Jul 8/Jul 9). Prior: 67% (2/3, Jul 22) — BAR CLEARED, 4th clearance since Jun 29; 0% (0/1, Jul 21, smallest fresh sample in the corpus's history); 33% (1/3, Jul 20); 25% (1/4, Jul 19); 25% (1/4, Jul 18); 80% (4/5, Jul 17); 100% (2/2, Jul 16, small-n); no fresh drafts Jul 15; 33% (1/3, Jul 14); no fresh drafts Jul 12–13; 0% (0/2, Jul 11); 33% (1/3, Jul 10); 50% (1/2, Jul 9, not a majority); 50% (4/8, Jul 8); 33% (2/6, Jul 7); no fresh drafts Jul 6; 20% (1/5, Jul 5); 20% (2/10, Jul 4); 33% (1/3, Jul 3); 0% (0/3, Jul 2); 0% (0/4, Jul 1); 22% (2/9 non-stale, Jun 30); 80% Jun 29 [BAR CLEARED]. |
| Resumption bar | majority A (>50%) sustained — **cleared Jun 29 (80%, n=5); Jun 30 returned 22%; Jul 1 0% (n=4); Jul 2 0% (n=3); Jul 3 33% (n=3); Jul 4 20% (n=10); Jul 5 20% (n=5); Jul 6 no fresh drafts; Jul 7 33% (n=6); Jul 8 50% (n=8, not a majority); Jul 9 50% (n=2, not a majority); Jul 10 33% (n=3); Jul 11 0% (n=2); Jul 12–13 no fresh drafts; Jul 14 33% (n=3); Jul 15 no fresh drafts; Jul 16 100% (n=2, small-n); Jul 17 80% (n=5) — cleared again, first n≥5 confirmation since Jun 29; Jul 18 25% (n=4); Jul 19 25% (n=4); Jul 20 33% (n=3); Jul 21 0% (n=1); Jul 22 67% (n=3) — cleared a 4th time; Jul 23 50% (n=4) — not a majority; Jul 24 no fresh drafts; Jul 25 no fresh drafts; Jul 26 no fresh drafts; Jul 27 no fresh drafts; Jul 28 no fresh drafts (5th consecutive); Jul 29 no fresh drafts (6th consecutive); Jul 30 no fresh drafts (7th consecutive, queue reached 0); Jul 31 no fresh drafts (8th consecutive, billing outage directly confirmed in pipeline logs); Aug 1 no fresh drafts (9th consecutive, billing outage still open, no clean gap between the Jul 31 and Aug 1 suppressions windows — see below)**. |
| Gap | **2pp below majority** (50% vs. >50% required, most recent measured cycle Jul 23, n=4) — the smallest miss in the corpus's history, but still ✗ by the letter. No fresh measurement Jul 24–Aug 1 — and per the finding below, none is possible until the writer's API billing is restored. P_close, P_compound, A7, and A8 remain the highest-leverage unimplemented voice levers, all unchanged since Jul 23, but **none of them can be acted on empirically while the pipeline produces zero drafts.** |
| Posting | paused; operator decision pending — Jun 29 cleared bar (80%, n=5), Jul 16 cleared it again (100%, n=2, small-n), Jul 17 cleared it a 3rd time with the first large-n confirmation (80%, n=5), Jul 18–21 fell back below bar (25%/25%/33%/0%), Jul 22 cleared it a 4th time (67%, n=3), Jul 23 fell back to 50% (n=4, just short of majority), **Jul 24–Aug 1 no fresh drafts (9 consecutive, a new record).** Queue contracted from 14 (Jul 23) to 11 (Jul 24) to 5 (Jul 25) to 4 (Jul 26) to 2 (Jul 27) to 1 (Jul 28–29) to 0 (Jul 30) and **remains 0 (Jul 31–Aug 1)** — zero new drafts entered any of the nine cycles. **Root cause confirmed live and continuing:** the gist's own `run_history`/`suppressions` logs show the writer's exact error — `"Error code: 400 - ... Your credit balance is too low to access the Anthropic API"` — with the most recent `billing_cycle_abort` at 2026-08-01T13:32:11Z. The retained 100-entry suppressions window has now fully rolled over between the Jul 31 and Aug 1 grading pulls with the identical failure signature in both — the strongest evidence yet that the outage has run continuously since Jul 24, not intermittently. 6 `budget_exhausted` + 6 `billing_cycle_abort` suppressions appear in Aug 1's window alone (2026-07-31T17:22Z–2026-08-01T13:32Z), skipping 330 candidates. **This is a live, ongoing billing/infra blocker, not a voice-quality or triage regression** — posting cannot resume, and no further voice evidence can accumulate, until it's fixed. **Now 9 days unresolved since the Jul 24 outage began.** |
| Coverage | **638 cities × 180 countries** (was 613 × 179; +25 via PR #81) |
| Queue status | **0 pending as of Aug 1 grading, unchanged from Jul 31.** 117 total drafts of any status in the gist (77 rejected, 38 posted, 2 approved); most recent `created_at` still 2026-07-23T14:11:46Z (9 days). **0 staleness candidates (moot, nothing pending).** Bot at 0.9.108.1 per `main`'s `VERSION`; `THEHEAT_WRITER_SAMPLES=2` + `THEHEAT_CRITIC_REVISE_ENABLED=1` live (though inert while billing is down). **P_tier's tracking remains CLOSED** (unchanged, no fresh `absolute_extreme` draft to retest). Jul 31's unattributed secondary `401 Unauthorized` signal is now identified as `twitter_metrics`, matching the already-tracked #452 — resolved, not new. New low-priority anomaly: recurring GDACS GeoRSS schema-drift failure, off the critical path. |

## Active proposals

Ordered by leverage. Priority as of Aug 1 (unchanged from Jul 24–31 — zero fresh drafts for a
9th consecutive cycle means no new evidence to reorder against): **P_dust, P9, and P_tier remain
...
not tested Jul 24 through Aug 1 — no record-type draft among any of the nine days' 0 fresh
drafts) > **A8** (opener-skeleton axis at 5 cycles, last evidence Jul 23; not tested Jul 24–Aug 1) >
**A7** (not tested Jul 22 through Aug 1) > **P5** (last evidence Jul 23). **A4, A5, A6, and A9 not
tested this cycle** (zero fresh drafts, 9th consecutive).
...
confirmed live in the pipeline's own logs, now with 9 consecutive days of continuous evidence.**
...
`billing_cycle_abort` at 2026-08-01T13:32:11Z — ~95 minutes before this grading run. The retained
100-entry suppressions window has now fully rolled over between the Jul 31 and Aug 1 pulls with
the identical failure signature both times (6 `budget_exhausted` + 6 `billing_cycle_abort` events,
330 candidates skipped in this window alone) — the strongest evidence yet that the outage has run
continuously since Jul 24 rather than intermittently. No `main` commit has landed since `4b4d965`
(Jul 24) — now 8+ days silent itself. This retroactively explains the queue-contraction pattern
tracked across the week of Jul 24–29 (queue drained from 14 to 0 with no replacement drafts): the
"oldest survivor drops first" shape those cycles kept re-confirming is consistent with the
ordinary per-type pending TTL sweep (fast 7d default) draining a queue nothing has replenished in
over a week, not a mysterious or newly-broken contraction mechanism. **No voice proposal below can
accumulate further evidence, and posting cannot resume, until billing is restored — now the
plan's single highest-priority item for 9 consecutive days running.**
