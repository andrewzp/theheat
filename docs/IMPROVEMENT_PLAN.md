...
| | |
|---|---|
| Bot commit | `13f8d64` (source-to-triage gateway merged on main; daily grading state reflects the May 22 no-fresh-drafts observation) |
...
| Last cycle A-rate | **21%** (3/14 fresh drafts, 2026-05-19; no fresh drafts 2026-05-22 — queue static since May 15) |
| Resumption bar | majority A (>50%) sustained |
| Gap | **29 pp** (50% − 21%; unchanged — no fresh drafts 2026-05-22) |
...
**Last seen:** May 14. **Retirement watch:** no new cold-record signals have reached the
queue since May 14 — 7 grading cycles (May 15, 16, 17, 18, 19, 22, and this one) without
observing the failure mode in fresh drafts. However, "no fresh drafts" cycles (May 22)
and cycles without cold-record signals are ambiguous under the 3-cycle rule (the rule
applies to cycles with drafts where the failure mode COULD appear but didn't). Bethel
draft remains in queue as the original exemplar; the fix has not shipped. Withholding
retirement until the next cold-record draft appears and either confirms or disconfirms
the pattern.
