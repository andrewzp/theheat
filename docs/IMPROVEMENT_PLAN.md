...
| | |
|---|---|
| Bot commit | `0.9.108.1` per `main`'s `VERSION` file (unchanged since checked 2026-07-16). No new `main` commits touching `src/two_bot/prompts/writer_prompt.py` observed this session (rebase onto fresh `main` was a no-op — `main`'s tip is unchanged at `#449` since Jul 17's rebase, confirmed again Jul 23 and Jul 24). |
...
| Last cycle A-rate | **— (0 fresh drafts, Jul 24).** Most recent measured: **50% (2/4, Jul 23) — bar NOT cleared** (not a majority; same convention as Jul 8/Jul 9). Prior: 67% (2/3, Jul 22) — BAR CLEARED, 4th clearance since Jun 29; 0% (0/1, Jul 21, smallest fresh sample in the corpus's history); 33% (1/3, Jul 20); 25% (1/4, Jul 19); 25% (1/4, Jul 18); 80% (4/5, Jul 17); 100% (2/2, Jul 16, small-n); no fresh drafts Jul 15; 33% (1/3, Jul 14); no fresh drafts Jul 12–13; 0% (0/2, Jul 11); 33% (1/3, Jul 10); 50% (1/2, Jul 9, not a majority); 50% (4/8, Jul 8); 33% (2/6, Jul 7); no fresh drafts Jul 6; 20% (1/5, Jul 5); 20% (2/10, Jul 4); 33% (1/3, Jul 3); 0% (0/3, Jul 2); 0% (0/4, Jul 1); 22% (2/9 non-stale, Jun 30); 80% Jun 29 [BAR CLEARED]. |
| Resumption bar | majority A (>50%) sustained — **cleared Jun 29 (80%, n=5); Jun 30 returned 22%; Jul 1 0% (n=4); Jul 2 0% (n=3); Jul 3 33% (n=3); Jul 4 20% (n=10); Jul 5 20% (n=5); Jul 6 no fresh drafts; Jul 7 33% (n=6); Jul 8 50% (n=8, not a majority); Jul 9 50% (n=2, not a majority); Jul 10 33% (n=3); Jul 11 0% (n=2); Jul 12–13 no fresh drafts; Jul 14 33% (n=3); Jul 15 no fresh drafts; Jul 16 100% (n=2, small-n); Jul 17 80% (n=5) — cleared again, first n≥5 confirmation since Jun 29; Jul 18 25% (n=4); Jul 19 25% (n=4); Jul 20 33% (n=3); Jul 21 0% (n=1); Jul 22 67% (n=3) — cleared a 4th time; Jul 23 50% (n=4) — not a majority; Jul 24 no fresh drafts to measure**. |
| Gap | **2pp below majority** (50% vs. >50% required, most recent measured cycle Jul 23, n=4) — the smallest miss in the corpus's history, but still ✗ by the letter. No fresh measurement Jul 24. P_close, P_compound, A7, and A8 remain the highest-leverage unimplemented levers, all unchanged since Jul 23 (no fresh evidence Jul 24). |
| Posting | paused; operator decision pending — Jun 29 cleared bar (80%, n=5), Jul 16 cleared it again (100%, n=2, small-n), Jul 17 cleared it a 3rd time with the first large-n confirmation (80%, n=5), Jul 18–21 fell back below bar (25%/25%/33%/0%), Jul 22 cleared it a 4th time (67%, n=3), Jul 23 fell back to 50% (n=4, just short of majority), **Jul 24 no fresh drafts**. Queue contracted from 14 (Jul 23) to 11 (Jul 24) — zero new drafts entered, 3 of Jul 23's 14 dropped out (Deaver A-, Al Basrah B+, Pikine A-). **0 strict staleness bulk-reject candidates this cycle** — 2 new watch items (Doha ~37.1h, Al Başrah Al Qadīmah ~32.6h, both forecast-date-elapsed under 48h). |
| Coverage | **638 cities × 180 countries** (was 613 × 179; +25 via PR #81) |
| Queue status | **11 pending as of Jul 24 grading, down from Jul 23's 14** — all 11 are an exact subset of Jul 23's graded batch (same `draft_id`s/scores/text), no re-grading performed: Delhi `air_quality_hazard` B+ (Jul 17), Anchorage `precipitation_extreme` B (Jul 18), Astana `precipitation_extreme` B+ (Jul 18), Wausaukee `all_time_high` A- (Jul 18), W Allis `all_time_high` B+ (Jul 19), Taiz `dust_event` B+ (Jul 20), Riyadh `dust_event` B (Jul 20), Barrow `precipitation_extreme` B+ (Jul 21), Doha `absolute_extreme` B+ (Jul 23), Al Başrah Al Qadīmah `absolute_extreme` A- (Jul 23), Capitol Hill 1 `record` C+ (Jul 23). **3 of Jul 23's 14 dropped from the queue, cause unconfirmed:** Deaver, Wyoming `all_time_high` A- (carried 7 consecutive cycles since Jul 17, the queue's longest-lived draft), Al Basrah, Iraq `absolute_extreme` B+ (carried 2 cycles), and **Pikine, Senegal `monthly_high` A- — graded fresh Jul 23 and already gone Jul 24**, the first same-day disappearance the recurring contraction pattern has produced. **0 strict staleness candidates this cycle**; 2 new watch items (Doha, Al Başrah Al Qadīmah, both forecast-date-elapsed under 48h). Bot at 0.9.108.1 per `main`'s `VERSION`; `THEHEAT_WRITER_SAMPLES=2` + `THEHEAT_CRITIC_REVISE_ENABLED=1` live. **P_tier's tracking remains CLOSED** (unchanged, no fresh `absolute_extreme` draft to retest). |

## Active proposals

Ordered by leverage. Priority as of Jul 24 (unchanged from Jul 23 — zero fresh drafts means no new
evidence to reorder against): **P_dust, P9, and P_tier remain CONFIRMED, tracking closed**
(unchanged). The remaining *unimplemented* active proposals, in order, carried forward verbatim
from Jul 23's recency logic: **P_close** (30 cycles, last evidence Jul 23: 2 positive — Al Başrah
Al Qadīmah, Pikine; 1 partial — Doha; 1 hedged-weak — Capitol Hill) > **P_compound** (re-activated
Jul 23, 15 cycles cumulative, not tested Jul 24 — no record-type draft among the day's 0 fresh
drafts) > **A8** (opener-skeleton axis at 5 cycles, last evidence Jul 23; not tested Jul 24) >
**A7** (not tested Jul 22 or Jul 23 or Jul 24) > **P5** (last evidence Jul 23). **A4, A5, A6, and
A9 not tested this cycle** (zero fresh drafts). **One operational item, not a voice proposal, now
escalated:** 3 more drafts dropped from the queue since Jul 23 — Deaver, Wyoming `all_time_high`
A- (carried 7 cycles), Al Basrah, Iraq `absolute_extreme` B+ (carried 2 cycles), and Pikine,
Senegal `monthly_high` A- (**graded fresh Jul 23, gone Jul 24** — the first same-day disappearance
this recurring contraction pattern has produced, following Jul 22→23's removal of 2 A-grades in a
single day). The pattern has moved from "costs a cycle's best carry-over" to "can erase a draft
before its grade has even aged one day" — worth the operator's priority attention over the voice
proposals below, since no amount of prompt tuning helps if graded drafts vanish before they can be
posted or even re-evaluated.
