...
| | |
|---|---|
| Bot commit | `0.9.108.1` per `main`'s `VERSION` file (unchanged since checked 2026-07-16). Rebase onto fresh `main` conflicted this session (first attempt since Jul 17 to hit a conflict — `main` has advanced past `#449` with unrelated economics/ops commits [#385, #449, #466] that also touch `docs/IMPROVEMENT_PLAN.md`); rebase aborted per the runbook fallback, `daily-plan-current` continues unchanged from its Jul 24 tip. No `main` commit touching `src/two_bot/prompts/writer_prompt.py` has been confirmed since Jul 16. |
...
| Last cycle A-rate | **— (0 fresh drafts, Jul 25 — 2nd consecutive zero-fresh cycle).** Most recent measured: **50% (2/4, Jul 23) — bar NOT cleared** (not a majority; same convention as Jul 8/Jul 9). Prior: 67% (2/3, Jul 22) — BAR CLEARED, 4th clearance since Jun 29; 0% (0/1, Jul 21, smallest fresh sample in the corpus's history); 33% (1/3, Jul 20); 25% (1/4, Jul 19); 25% (1/4, Jul 18); 80% (4/5, Jul 17); 100% (2/2, Jul 16, small-n); no fresh drafts Jul 15; 33% (1/3, Jul 14); no fresh drafts Jul 12–13; 0% (0/2, Jul 11); 33% (1/3, Jul 10); 50% (1/2, Jul 9, not a majority); 50% (4/8, Jul 8); 33% (2/6, Jul 7); no fresh drafts Jul 6; 20% (1/5, Jul 5); 20% (2/10, Jul 4); 33% (1/3, Jul 3); 0% (0/3, Jul 2); 0% (0/4, Jul 1); 22% (2/9 non-stale, Jun 30); 80% Jun 29 [BAR CLEARED]. |
| Resumption bar | majority A (>50%) sustained — **cleared Jun 29 (80%, n=5); Jun 30 returned 22%; Jul 1 0% (n=4); Jul 2 0% (n=3); Jul 3 33% (n=3); Jul 4 20% (n=10); Jul 5 20% (n=5); Jul 6 no fresh drafts; Jul 7 33% (n=6); Jul 8 50% (n=8, not a majority); Jul 9 50% (n=2, not a majority); Jul 10 33% (n=3); Jul 11 0% (n=2); Jul 12–13 no fresh drafts; Jul 14 33% (n=3); Jul 15 no fresh drafts; Jul 16 100% (n=2, small-n); Jul 17 80% (n=5) — cleared again, first n≥5 confirmation since Jun 29; Jul 18 25% (n=4); Jul 19 25% (n=4); Jul 20 33% (n=3); Jul 21 0% (n=1); Jul 22 67% (n=3) — cleared a 4th time; Jul 23 50% (n=4) — not a majority; Jul 24 no fresh drafts; Jul 25 no fresh drafts (2nd consecutive)**. |
| Gap | **2pp below majority** (50% vs. >50% required, most recent measured cycle Jul 23, n=4) — the smallest miss in the corpus's history, but still ✗ by the letter. No fresh measurement Jul 24 or Jul 25. P_close, P_compound, A7, and A8 remain the highest-leverage unimplemented levers, all unchanged since Jul 23 (no fresh evidence Jul 24 or Jul 25). |
| Posting | paused; operator decision pending — Jun 29 cleared bar (80%, n=5), Jul 16 cleared it again (100%, n=2, small-n), Jul 17 cleared it a 3rd time with the first large-n confirmation (80%, n=5), Jul 18–21 fell back below bar (25%/25%/33%/0%), Jul 22 cleared it a 4th time (67%, n=3), Jul 23 fell back to 50% (n=4, just short of majority), **Jul 24–25 no fresh drafts (2 consecutive)**. Queue contracted from 14 (Jul 23) to 11 (Jul 24) to **5 (Jul 25)** — zero new drafts entered either cycle; 3 of Jul 23's 14 dropped Jul 24 (Deaver A-, Al Basrah B+, Pikine A-), then **6 more of Jul 24's 11 dropped Jul 25** (Delhi B+, Wausaukee A-, Anchorage B, Astana B+, Doha B+, Al Başrah Al Qadīmah A-) — the largest single-cycle contraction recorded, roughly double the prior record. **0 strict staleness bulk-reject candidates this cycle** — all 5 remaining drafts are past-tense/historical, none forecast-elapsed. |
| Coverage | **638 cities × 180 countries** (was 613 × 179; +25 via PR #81) |
| Queue status | **5 pending as of Jul 25 grading, down sharply from Jul 24's 11** — all 5 are an exact subset of Jul 19–23's graded batch (same `draft_id`s/scores/text), no re-grading performed: W Allis `all_time_high` B+ (Jul 19), Taiz `dust_event` B+ (Jul 20), Riyadh `dust_event` B (Jul 20), Barrow `precipitation_extreme` B+ (Jul 21), Capitol Hill 1 `record` C+ (Jul 23). **6 of Jul 24's 11 dropped from the queue, cause unconfirmed:** Delhi `air_quality_hazard` B+, Wausaukee `all_time_high` A-, Anchorage `precipitation_extreme` B, Astana `precipitation_extreme` B+ (all four carried since Jul 18 — 7 cycles each, dropping together for the first time), plus Doha `absolute_extreme` B+ and Al Başrah Al Qadīmah `absolute_extreme` A- (both flagged as forecast-elapsed watch items at Jul 24's grading, gone within 2 days). **0 strict staleness candidates this cycle.** Bot at 0.9.108.1 per `main`'s `VERSION`; `THEHEAT_WRITER_SAMPLES=2` + `THEHEAT_CRITIC_REVISE_ENABLED=1` live. **P_tier's tracking remains CLOSED** (unchanged, no fresh `absolute_extreme` draft to retest). |

## Active proposals

Ordered by leverage. Priority as of Jul 25 (unchanged from Jul 24 — zero fresh drafts for a 2nd
...
not tested Jul 24 or Jul 25 — no record-type draft among either day's 0 fresh drafts) > **A8**
(opener-skeleton axis at 5 cycles, last evidence Jul 23; not tested Jul 24 or Jul 25) > **A7** (not
tested Jul 22, Jul 23, Jul 24, or Jul 25) > **P5** (last evidence Jul 23). **A4, A5, A6, and A9 not
tested this cycle** (zero fresh drafts, 2nd consecutive). **One operational item, not a voice
proposal, now the plan's top priority regardless of ranking:** the queue-contraction pattern
roughly doubled in size this cycle (11→5, -6) after Jul 23→24 set the prior record at -3. This
cycle removed four 7-cycle-old carry-overs together (Delhi, Wausaukee, Anchorage, Astana — all
stable since Jul 18) plus both of Jul 24's forecast-elapsed watch items (Doha, Al Başrah Al
Qadīmah), taking 2 A-grades and 2 B/B+ grades out of the queue with zero fresh drafts to replace
them. The pattern has now escalated two cycles running (Jul 23→24: -3, one same-day A-grade loss;
Jul 24→25: -6, four long-lived carry-overs at once) — worth the operator's priority attention over
every voice proposal below, since no amount of prompt tuning helps if graded drafts vanish before
they can be posted or even re-evaluated, and the queue may empty entirely within 1-2 more cycles
at the current drain rate with no fresh drafts arriving to offset it.
