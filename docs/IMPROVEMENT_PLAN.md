...
| Last cycle A-rate | **— (0 fresh drafts, Jul 31 — 8th consecutive zero-fresh cycle, dry spell now a full 8 days).** Most recent measured: **50% (2/4, Jul 23) — bar NOT cleared** (not a majority; same convention as Jul 8/Jul 9). Prior: 67% (2/3, Jul 22) — BAR CLEARED, 4th clearance since Jun 29; 0% (0/1, Jul 21, smallest fresh sample in the corpus's history); 33% (1/3, Jul 20); 25% (1/4, Jul 19); 25% (1/4, Jul 18); 80% (4/5, Jul 17); 100% (2/2, Jul 16, small-n); no fresh drafts Jul 15; 33% (1/3, Jul 14); no fresh drafts Jul 12–13; 0% (0/2, Jul 11); 33% (1/3, Jul 10); 50% (1/2, Jul 9, not a majority); 50% (4/8, Jul 8); 33% (2/6, Jul 7); no fresh drafts Jul 6; 20% (1/5, Jul 5); 20% (2/10, Jul 4); 33% (1/3, Jul 3); 0% (0/3, Jul 2); 0% (0/4, Jul 1); 22% (2/9 non-stale, Jun 30); 80% Jun 29 [BAR CLEARED]. |
| Resumption bar | majority A (>50%) sustained — **cleared Jun 29 (80%, n=5); Jun 30 returned 22%; Jul 1 0% (n=4); Jul 2 0% (n=3); Jul 3 33% (n=3); Jul 4 20% (n=10); Jul 5 20% (n=5); Jul 6 no fresh drafts; Jul 7 33% (n=6); Jul 8 50% (n=8, not a majority); Jul 9 50% (n=2, not a majority); Jul 10 33% (n=3); Jul 11 0% (n=2); Jul 12–13 no fresh drafts; Jul 14 33% (n=3); Jul 15 no fresh drafts; Jul 16 100% (n=2, small-n); Jul 17 80% (n=5) — cleared again, first n≥5 confirmation since Jun 29; Jul 18 25% (n=4); Jul 19 25% (n=4); Jul 20 33% (n=3); Jul 21 0% (n=1); Jul 22 67% (n=3) — cleared a 4th time; Jul 23 50% (n=4) — not a majority; Jul 24 no fresh drafts; Jul 25 no fresh drafts; Jul 26 no fresh drafts; Jul 27 no fresh drafts; Jul 28 no fresh drafts (5th consecutive); Jul 29 no fresh drafts (6th consecutive); Jul 30 no fresh drafts (7th consecutive, queue reached 0); Jul 31 no fresh drafts (8th consecutive, billing outage now directly confirmed in pipeline logs — see below)**. |
| Gap | **2pp below majority** (50% vs. >50% required, most recent measured cycle Jul 23, n=4) — the smallest miss in the corpus's history, but still ✗ by the letter. No fresh measurement Jul 24–31 — and per the finding below, none is possible until the writer's API billing is restored. P_close, P_compound, A7, and A8 remain the highest-leverage unimplemented voice levers, all unchanged since Jul 23, but **none of them can be acted on empirically while the pipeline produces zero drafts.** |
| Posting | paused; operator decision pending — Jun 29 cleared bar (80%, n=5), Jul 16 cleared it again (100%, n=2, small-n), Jul 17 cleared it a 3rd time with the first large-n confirmation (80%, n=5), Jul 18–21 fell back below bar (25%/25%/33%/0%), Jul 22 cleared it a 4th time (67%, n=3), Jul 23 fell back to 50% (n=4, just short of majority), **Jul 24–31 no fresh drafts (8 consecutive, a new record).** Queue contracted from 14 (Jul 23) to 11 (Jul 24) to 5 (Jul 25) to 4 (Jul 26) to 2 (Jul 27) to 1 (Jul 28–29) to 0 (Jul 30) and **remains 0 (Jul 31)** — zero new drafts entered any of the eight cycles. **Root cause directly confirmed this cycle** (previously only inferred from `BRIEFING.md`'s prose): the gist's own `run_history`/`suppressions` logs show the writer's exact error — `"Error code: 400 - ... Your credit balance is too low to access the Anthropic API"` — and a `billing_cycle_abort` event as recently as 2026-07-31T14:15:44Z reading `"cycle aborted after budget_exhausted ...; 58 queued candidate(s) skipped."` 7 `budget_exhausted` + 7 `billing_cycle_abort` suppressions appear in the last 100, spanning Jul 30–31, skipping 41–292 candidates per aborted cycle. **This is a live, ongoing billing/infra blocker, not a voice-quality or triage regression** — posting cannot resume, and no further voice evidence can accumulate, until it's fixed. |
| Coverage | **638 cities × 180 countries** (was 613 × 179; +25 via PR #81) |
| Queue status | **0 pending as of Jul 31 grading, unchanged from Jul 30.** 122 total drafts of any status in the gist (82 rejected, 38 posted, 2 approved); most recent `created_at` still 2026-07-23T14:11:46Z (8 days). **0 staleness candidates (moot, nothing pending).** Bot at 0.9.108.1 per `main`'s `VERSION`; `THEHEAT_WRITER_SAMPLES=2` + `THEHEAT_CRITIC_REVISE_ENABLED=1` live (though inert while billing is down). **P_tier's tracking remains CLOSED** (unchanged, no fresh `absolute_extreme` draft to retest). |

## Active proposals

Ordered by leverage. Priority as of Jul 31 (unchanged from Jul 24–30 — zero fresh drafts for an
8th consecutive cycle means no new evidence to reorder against): **P_dust, P9, and P_tier remain
...
not tested Jul 24 through Jul 31 — no record-type draft among any of the eight days' 0 fresh
drafts) > **A8** (opener-skeleton axis at 5 cycles, last evidence Jul 23; not tested Jul 24–31) >
**A7** (not tested Jul 22 through Jul 31) > **P5** (last evidence Jul 23). **A4, A5, A6, and A9 not
tested this cycle** (zero fresh drafts, 8th consecutive).

**One operational item, not a voice proposal, still supersedes every ranking below and is the
plan's sole top priority: the pipeline has no Anthropic API billing credit and cannot draft — now
directly confirmed in the pipeline's own logs, not just inferred from `BRIEFING.md`.** The gist's
`run_history` (last 20 runs) and `suppressions` (last 100) logs, read in full for the first time
this cycle, carry the writer's exact HTTP 400 error (`"Your credit balance is too low to access
the Anthropic API"`) and 7 `budget_exhausted` + 7 `billing_cycle_abort` events between Jul 30
10:19Z and Jul 31 14:15Z — the most recent just ~75 minutes before this grading run. Each abort
discards the entire remaining triage batch (41–292 candidates skipped per event), so this isn't a
quiet stall: real signal is being generated and thrown away every cycle. No `main` commit has
landed since `4b4d965` (Jul 24) — now 7+ days silent itself. This retroactively explains the
queue-contraction pattern tracked across the prior week's entries here (Jul 24→25 -3→-6, Jul
25→26 -1, Jul 26→27 -2, Jul 27→28 -1, Jul 28→29 flat, Jul 29→30 the last draft finally rejected,
**Jul 30→31 holds at 0**): the "oldest survivor drops first" shape those cycles kept re-confirming
is consistent with the ordinary per-type pending TTL sweep (fast 7d default) draining a queue
nothing has replenished in over a week, not a mysterious or newly-broken contraction mechanism. **No voice
proposal below can accumulate further evidence, and posting cannot resume, until billing is
restored** — flagging this prominently is this cycle's single highest-value contribution to the
plan.
