...
| 2026-06-07 | 0 | — | — | — | — | **N/A** | — | No fresh drafts; queue empty. 13 carry-overs auto-rejected by 7d TTL sweep (0.9.6.0) during June 1–6 active-bot window (oldest: 20–23 days old at clearance). Routine restored after 12-day DOWN gap (May 26 → June 7). Bot re-enabled June 1. 0.9.15.0 (gpm S3 single-request daily-grid feed) shipped June 6. Check suppression ledger for `evidence_contract` + `triage_cap` kill counts in June 1–7 window — 0 new pending suggests filtering still active post-reactivation. `gh` CLI absent (11th consecutive staleness skip). |
