...
The **52-test focused offline suite** covers evidence classification, exact byte retention, altered/missing manifests and payloads, receipt identity, unsafe paths, failed creation, immutable existing packages, case/text joins and reading the named Git commit despite a changed worktree. Independent review caught two gaps: a matching snapshot hash alone did not prove the corpus row came from that snapshot, and exponent overflow could bypass nonfinite-JSON rejection. Both are fixed. Exact event/text/status/receipt/publication-time joins now verify every corpus row against the original source; generation timestamps remain distinct from publication times. Nonfinite numbers, including `1e999`, fail in all three JSON inputs. Ruff and mypy pass. The real local package has been verified unchanged against the independent digest above.
