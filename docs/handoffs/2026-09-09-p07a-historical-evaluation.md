...
3. **Synthetic text through current pipeline gates.** The writer result and a completed safety-model result are injected inside restoring local patches, and the writer sample count is fixed at one. The production safety regex, honesty checks and factual checks run. No safety-model or writer performance is evaluated. Controls stop at the intercepted checker boundary and remain `verification_required`. P07 made safety mandatory, replacing this harness's former missing-key shortcut with an explicit model stub; production missing-key behavior is never treated as a pass.
