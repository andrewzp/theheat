...
- Document the observed-temperature routing and evidence gaps for Spain, India and South Africa. Existing station inventory is not treated as a qualified pilot or a measure of global coverage.
...
session became a product-level reckoning about why @theheat reads silent on the stories
that matter (out of Anthropic credits; good drafts dying in an unwatched manual queue;
selection blind to newsworthiness). Full session record:
[docs/handoffs/2026-07-03.md](/Users/andrewpuschel/Documents/Claude/theheat/docs/handoffs/2026-07-03.md).

- **reganom writer-voice upgrade** ([#349](https://github.com/andrewzp/theheat/pull/349)). Rewrote the "Regional anomaly bundles" section of [writer_prompt.py](/Users/andrewpuschel/Documents/Claude/theheat/src/two_bot/prompts/writer_prompt.py) to lead with significance, weave the sampled-cities attribution, round the headline, land σ as awe, and name the stakes. Safe rounding: the bundle ([temperature.py](/Users/andrewpuschel/Documents/Claude/theheat/src/two_bot/intern/temperature.py)) pre-publishes `headline_metric.value_rounded_c = round(mean_anomaly_c)` so the writer cites a bundle value verbatim (no BUNDLE_FACT precision-kill). Fact-check ([fact_check_prompt.py](/Users/andrewpuschel/Documents/Claude/theheat/src/two_bot/prompts/fact_check_prompt.py)) rule **g** blesses the rounded value (whole degree must equal `value_rounded_c`; decimal within ±0.1°C of raw) and **new rule j3** rejects overnight-low / nighttime-relief / health / infrastructure claims (reganom is daily-max only). New offline harness [scripts/reganom_writer_dryrun.py](/Users/andrewpuschel/Documents/Claude/theheat/scripts/reganom_writer_dryrun.py) + workflow_dispatch [.github/workflows/reganom-dryrun.yml](/Users/andrewpuschel/Documents/Claude/theheat/.github/workflows/reganom-dryrun.yml) run the live writer→critic chain in Actions. codex-xhigh 3 rounds → clean APPROVE. **Follow-up (branch `fix/reganom-stakes-attribution`, not merged):** the live dispatch surfaced that the merged exemplar personifies "France" (killed by the fact-check as a bare-region generalization); the branch re-anchors the stakes clause to the sampled cities (live-verified 3/3 shipping) — needs codex + a PR.
- **Autoship allowlist → verifiable record types** ([#352](https://github.com/andrewzp/theheat/pull/352)). Good drafts (a Prudhoe Bay all-time high, a marine-heatwave record) were dying unposted because `AUTOSHIP_ALLOWLIST` ([approval.py](/Users/andrewpuschel/Documents/Claude/theheat/src/editorial/approval.py)) was only `{hot10, co2_milestone, ch4_milestone}`. Principle: auto-ship verifiable RECORDS, keep anomalies + human-impact manual. Added `all_time_high/low`, `monthly_high/low`, `marine_heatwave`. Kept manual: calendar records (volume), `regional_anomaly` + anomalies (interpretive), `country_high/low` (human polish window), all human-impact. Kill-switch unchanged (`THEHEAT_AUTOSHIP_ON_CRITIC_PASS=0`). codex-xhigh APPROVE; +8 tests.
- **Operational (not code):** the writer is the metered Anthropic API (separate from the Claude Code Max plan) and its **credit balance hit zero** — the writer stopped drafting while runs stayed "success" (graceful BudgetExhaustedError); voice-regression went red on billing 400s. **Resolved** — Andrew topped up; writer confirmed back. The 10 drafts stuck in the pre-#352 manual queue won't retroactively auto-ship (approve/reject them by hand once).
- **Next: Bet A** — Andrew chose to make selection autonomously smart about newsworthiness + sourced human impact (over a human-curated approach). Brainstorm in progress, 3 decisions locked: [docs/superpowers/specs/2026-07-03-newsworthiness-bet-a-design.md](/Users/andrewpuschel/Documents/Claude/theheat/docs/superpowers/specs/2026-07-03-newsworthiness-bet-a-design.md).

### reganom recency window + ended-spell honesty + world-half eval-gating fix (2026-06-28)

Continued the regional-heatwave work from 2026-06-27. The France/UK spell was real and huge
(live ERA5: France +12–14°C for 7 days Jun 21–27; UK +7–13°C) but had **eased by one day** by
the time the onset guard was cleared, and reganom only fired on spells *still qualifying on
the latest ERA5 day* — so a week-long +13°C event produced no draft. Full session record:
[docs/handoffs/2026-06-28.md](/Users/andrewpuschel/Documents/Claude/theheat/docs/handoffs/2026-06-28.md).

- **reganom 2-day recency window + ended-spell honesty** ([#347](https://github.com/andrewzp/theheat/pull/347)). `detect_regional_anomaly` ([src/data/reanalysis_anomaly.py](/Users/andrewpuschel/Documents/Claude/theheat/src/data/reanalysis_anomaly.py)) now enumerates all maximal qualifying runs and fires on the **most-recent** sustained (≥`min_days`) run whose last day is within `RECENCY_DAYS=2` of the latest complete day — a *just-ended* heatwave still drafts, not only one ongoing today. The magnitude bar (**+6°C / ≥2σ / ≥50% support**) is unchanged; only recency widened. Robust against an isolated recent spike masking a genuine just-ended spell. **Ended-spell honesty** (codex P2, 3 review rounds): the event carries `latest_complete_day`; the bundle ([temperature.py](/Users/andrewpuschel/Documents/Claude/theheat/src/two_bot/intern/temperature.py)) computes `ended_days_ago` + exposes the window; the writer prompt requires **past tense + the window** when ended; fact-check rule **j2** ([fact_check_prompt.py](/Users/andrewpuschel/Documents/Claude/theheat/src/two_bot/prompts/fact_check_prompt.py)) is an LLM recency gate (catches present-tense verbs substring lists miss); and when ended the bundle appends present-tense phrasings to `forbidden_claims`. Result: the **France draft is in the `manual_only` queue** ("ran 11.53°C above the 1991–2020 daily normal for 8 straight days through June 27 — 2.8σ"). UK & Ireland was generated but killed for length (>280 chars) and is onset-suppressed. codex-xhigh APPROVE (looped 3 rounds); new reganom recency + honesty tests.
- **World-half eval-gating fix** ([#345](https://github.com/andrewzp/theheat/pull/345)) — closes the PENDING defect from 2026-06-27. `_run_world_cached_half` ([src/orchestrator/sources/open_meteo.py](/Users/andrewpuschel/Documents/Claude/theheat/src/orchestrator/sources/open_meteo.py)) warmed (archive endpoint) before evaluating (forecast endpoint) under one shared `saturated` flag, so the archive 429 skipped evaluation entirely (`cov:0.0` every bootstrap run — zero record detection during the ~2-week warm-up). Reworked into three phases under one cache read/write: **EVAL** (forecast; collect only) → **WARM** (archive; refresh stale) → **CONSOLIDATE** (re-evaluate every pending city vs the freshest thresholds, finalize `om_bundles`/country records, stamp provisional). Saturation split into `eval_saturated`/`warm_saturated` (eval gated only on its own); `classify_world_status` ([src/orchestrator/world_cache.py](/Users/andrewpuschel/Documents/Claude/theheat/src/orchestrator/world_cache.py)) rewritten (warm saturation tolerated only during healthy bootstrap growth; steady-state keyed to **pre-run** entry count); `prev_cached` from the true pre-run entry count, not the lagging `_meta.cached_count`; refute-before-emit + freshness-honest `as_of` preserved. codex round 1 REJECT caught two real defects (`prev_cached`/`_meta` lag, falsy-`as_of` false-freshness) → fixed → round 2 APPROVE. +20 tests ([tests/test_world_eval_gating.py](/Users/andrewpuschel/Documents/Claude/theheat/tests/test_world_eval_gating.py)). **Rollout:** the next `provider=both` runs should show `cov:` nonzero-and-climbing (read from `run_history[].sources[].details.open_meteo_pipeline_metrics`, NOT `source_health.runs[].error` which is null on a `success` run).
- **Known follow-ups (open/held).** [#346](https://github.com/andrewzp/theheat/pull/346) (re-key the world pipeline by `city|country` so genuinely-distinct same-name cities — Barcelona ES/VE, Hyderabad IN/PK, Valencia ES/VE — stop conflating into false records) is **OPEN but HELD**: it must first dedup the 5 *same-place* country aliases (Dubai/Amsterdam/Hong Kong/Kinshasa/Brazzaville) in `data/cities.csv`, else re-keying splits them into duplicate cache entries → duplicate posts. **Next session P1 (no PR yet):** the reganom draft is honest but *undersells* the event — upgrade the reganom writer-prompt voice (lead with significance, weave the sampled-cities attribution, round the number, σ as awe, name the stakes; strengthen the bar, keep honesty). Deferred idea: weave real *sourced* news anecdotes into tweets (needs grounded retrieval — not the writer's imagination).

### Regional heatwave signal activated + reganom calibration + #338 follow-ups (2026-06-26/27)

Triggered by a viral "hottest day ever in France" tweet: @theheat was silent on the late-June 2026 European heatwave because it only fires on single-station records — a broad regional heatwave that breaks no one city's all-time record was invisible by design. The fix already existed, dormant. Full session record: [docs/handoffs/2026-06-27.md](/Users/andrewpuschel/Documents/Claude/theheat/docs/handoffs/2026-06-27.md).

- **Regional-anomaly signal ACTIVATED** (`THEHEAT_REGANOM_ENABLED=1`, repo variable; `manual_only`). `reanalysis_anomaly` ("reganom", [#203](https://github.com/andrewzp/theheat/pull/203), built 2026-06-09) detects when a curated region's sampled cities run **≥+6°C AND ≥2σ AND ≥50% point-support** above the 1991–2020 daily ERA5 normal for **≥3 consecutive days** — the heatwave class single-station records miss. It had shipped env-gated OFF and sat dark 17 days; the "operator verifies then turns it on" step never happened. A live dry-run of the real detector fires **France (+11.9°C mean / 6 cities / 7-day, z=2.9)** and **UK & Ireland (+9.9°C / 6-day)** right now. Honesty-gated to "N sampled cities in [Region]", never "[Region] averaged" — the claim/warrant principle, already implemented. Drafts are `manual_only`.
- **reganom draft-calibration fix** ([#343](https://github.com/andrewzp/theheat/pull/343)). The first live run detected France/UK but the fact-check honesty gate killed both drafts — rightly in one case, over-strictly in the other. Fixed **bundle-side so the gate itself is untouched** (`src/two_bot/intern/temperature.py`): dropped `fraction_exceeding` from `current_facts` (a fraction of city-*days* the writer mis-rendered as "5 of 6 cities"; kept in `raw_signal_dump`), and publish `mean_zscore` at 1 dp so the writer's natural "3.4σ" citation matches the bundle exactly. Writer prompt reinforced to cite `cities_sampled` as the only count. Regression tests added.
- **#338 review follow-ups** ([#341](https://github.com/andrewzp/theheat/pull/341)). A day-ceiling-isolating `OpenMeteoBudget` test (the prior "daily ceiling" test actually tripped the hourly guard — renamed + corrected), plus docstrings for `_run_world_cached_half` and `detect_country_records`' coverage accounting. No behavior change.
- **World threshold cache rollout watch (#338).** Warming started (first post-merge `alerts` run cached 6/596 cities incl. Madrid/Sevilla/Lyon) but reported `degraded` (`sat:True`): the warm path saturates Open-Meteo's *archive* endpoint after ~7 cities — a separate per-minute request-count limit the weight-based `OpenMeteoBudget` doesn't model. Read cached_count/`sat` from `source_health.open_meteo_extreme_signals.runs[].error`, not `run_history` details. **Known defect (PENDING):** `_run_world_cached_half` gates the forecast/evaluation phase on `not saturated`, so while warming saturates every bootstrap run the world half evaluates ZERO cached cities for records (`cov:0.0`). Fix queued — evaluate cached cities before warming stale ones, and cap the warm budget to the archive's real request limit.
...
- **Interim world-coverage budget for the Open-Meteo `both` half** (2026-06-26). First-run verification of `provider=both` found the world half **silently rate-limited**: Open-Meteo's free **archive** endpoint enforces a per-minute request-weight limit that a 30-year daily pull saturates after ~12 cities, so the live scan of ~595 non-US cities 429'd all but the first ~12 (`world[readings:12 failures:584]`) — and the failures were a silent internal counter, never a source failure, so monitoring stayed green. Reproduced locally: forecast endpoint 22/22 OK, archive endpoint 14×`200` then `429 "Minutely API request limit exceeded"`. Architectural cause: the GHCN/US half is backed by a precomputed threshold cache (`scripts/build_station_thresholds.py`) while the world half recomputes 30-year archive history *live every run*, with no Open-Meteo 429 retry (the retry helper only honors a hardcoded WAF host allowlist). **Interim guard:** `select_world_budget_cities` (`src/data/open_meteo.py`) caps the `both` world half to a `WORLD_FETCH_BUDGET=10`, urgent-first slice — `URGENT_WORLD_HEAT_CITIES` interleaves the 2026 European-heatwave cities (Madrid, Sevilla, Zaragoza, Lyon, Paris) with perennial hot-spots (Jacobabad, Kuwait City, Mecca, Delhi, Ahvaz) so the acute event is observed instead of starved by `prioritize_cities()`'s random tail. Runs drop from ~214s back to ~12s with no 429s; Spain×3/France×2 also re-enables country records. **Proper fix to follow:** a world threshold cache mirroring GHCN's, so per-run cost is a cheap forecast call and full ~595-city coverage returns (planned, codex-reviewed).
- **World threshold cache — the proper fix** ([#338](https://github.com/andrewzp/theheat/pull/338)). Gives the `both` world half the GHCN/US half's cached-threshold model so a run is a cheap forecast compare, not a live 30-yr archive pull. **Warm path** (`_run_world_cached_half`, `src/orchestrator/sources/open_meteo.py`) refreshes the ~8 stalest/missing cities' archives per run (urgent-first), paced by a process-level `OpenMeteoBudget` (`src/data/openmeteo_budget.py`, 600/min with leaderboard reserve); thresholds live in a **separate gist file** (`world_threshold_cache.json`, `state.json` untouched) with a **field/month-wise merge** (`src/orchestrator/world_cache.py`) that survives concurrent same-day writes. **Hot path** does a cheap multi-location forecast vs cached thresholds (`evaluate_city`, `src/data/world_thresholds.py`). An Open-Meteo **429 now raises `OpenMeteoSaturated`** and stops the run with `degraded` status — the original *silent* saturation is structurally impossible. Plus **anti-re-fire** (a fired record writes a provisional threshold so it can't re-tweet for 30 days), a **country-record coverage floor** (no false national record from a partially-warmed country), and a **failure classifier** (`classify_world_status`, with cross-run stall detection). `calendar_date`/`record_streak`/`simultaneous_records` become **US-only** for the world (gist-prohibitive to cache). The interim `select_world_budget_cities` cap is removed. Built 11-task subagent-driven TDD off a spec + plan each **2× codex-reviewed** (caught 4 P0 + 8 P1 — concurrent-merge clobber, the leaderboard-shared budget reality, anti-re-fire, country bootstrap, a real mean-count bug) + an **Opus whole-branch review** (READY TO MERGE; pacing proven safe under a production-scale simulation). Full suite 2081 passed, ruff + mypy clean. Design + plan: `docs/superpowers/specs/2026-06-26-world-threshold-cache-design.md`, `docs/superpowers/plans/2026-06-26-world-threshold-cache.md`. **Rollout:** the cache warms over ~10 days; watch `cached_count`/`coverage_ratio`/`saturated` in the `open_meteo_extreme_signals` source details (handoff 2026-06-26).
