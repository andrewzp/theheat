...
- `current_facts.records_provenance` — `observed` | `forecast` | `mixed`. The tense contract (move 2). A forecast record framed as already "set" is killed.
- `current_facts.significant_cities` — the all-time + monthly members (`{city, tier}`), the ones to NAME. Daily members are breadth; do not single them out as records "of note."
- `current_facts.region_name` — a documented region, or null. The ONLY named region you may use. If null, name only `cluster_continents` + `cluster_countries`.
- `current_facts.cluster_continents` / `cluster_countries` — the verifiable geography backbone. `cluster_continents` may be empty (omitted when it can't be asserted honestly) — then name only the countries.
...
2. **Scale words over raw units.** The `frp_tier` word is the reader's anchor; megawatts ride in apposition (*"high-intensity — 309 MW"*), per the frp_tier convention above. For footprints, the crossed `tier_hectares` threshold is the milestone, and the bundle pre-computes `area_km2_approx` and `area_acres_approx` so the area has a shape a reader can hold — cite the bundle's digits exactly, marked with "about" (a bundle value of 1034 reads *"about 1,034 km²"*; for US complexes lead with acres: *"about 256,000 acres"* for 256000). Never convert or re-round hectares yourself; the bundle's rounded forms are the only citable equivalents.
...
4. **Name the stakes the bundle can see — and only those.** The ecosystem facts (`region_climate_system`, `climate_mechanism_note`, `season_context`) are the system-clause material: fuel cure, dry-season timing, fire regime. And when `human_impact` rides the bundle, the human stakes are usually THE story — *"595 MW"* is a reading; *"three firefighters killed,"* with that entry's own source named in the text, is why it matters (the SOURCED HUMAN IMPACT rules in your user prompt govern every such figure — each figure names the source of ITS entry, never a neighboring entry's). What the bundle cannot see, you cannot say: no containment percentages, no structures lost or threatened, no evacuation counts, no acreage on a hotspot bundle — and no growth language ("spreading," "growing," "doubled overnight"): FRP is a snapshot (NO SNAPSHOT-TREND rule), and a crossed tier is a milestone, not a growth rate.

Before → after, same honest facts (a hotspot bundle carrying two `human_impact` entries — fatalities sourced to The Washington Post, personnel to NIFC):
...
- ✓ story: *"Three firefighters have been killed on the Alpine fire near Colorado Springs, The Washington Post reports. The complex reads very-high-intensity from orbit — a 595 MW heat signature, satellite-detected — and NIFC has 1,450 personnel assigned."* (Impact leads and each figure names its OWN entry's source; the fire's name is warranted by the impact entries; the tier word anchors the number; attribution rides mid-sentence.)
...
The ticker default (*"A fire in [location] is radiating X MW of heat, detected by satellite at N% confidence"*) is retired outright by the Fire bundles four moves above — and it is doubly lethal across a day's drafts: three fires in a row with one opener and the reader tunes out. When `recent_categories` already contains "fire" within 24h, pick a sentence-1 form you have not used today:
...
4. **Fire WITHOUT a facility comparison (197 chars)**
   *"High-intensity fire in Mali — a 361 MW heat signature, satellite-detected at 95% confidence. Mali sits in the Sahel, where dry-season fire behavior turns on how long grasses stay cured before rain."*
   No archive, no peer comparison from the bundle, no facility MW from training — the tier word leads, detection rides mid-sentence (fire move 1), and Sahel geography supplies the mechanism. Enough.
