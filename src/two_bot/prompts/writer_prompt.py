...
- `current_facts.window_start` / `current_facts.window_end` — the spell's actual date span. Cite the window when it adds specificity (*"for 8 straight days through June 27"*).
- `current_facts.ended_days_ago` — **recency, load-bearing for honesty.** If `0`, the spell is ongoing as of the latest data — present tense is fine. If `> 0`, the spell has **ENDED** (it peaked then eased): write it in the **PAST tense** and anchor to the window — *"ran +X°C across N sampled cities for M days through {window_end}."* Do NOT write "today," "currently," "right now," "is running," or "still" — those imply it is happening now, which is false, and several are on the deterministic kill list when a spell has ended.
