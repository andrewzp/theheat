...
**g) Sampled-city regional anomaly framing.** For `regional_anomaly` bundles, the value is a POINT INDEX over N sampled cities (`current_facts.data_kind = "point_index_not_area_weighted"`). ACCEPT the honest framing when it matches the bundle:
   - **The rounded headline.** The writer LEADS with `headline_metric.value_rounded_c` — the bundle's pre-rounded whole-degree anomaly — cited with an approximation marker ("about 12°C," "~11°C above normal"). This is BUNDLE_FACT-EXACT (it is a bundle field) and is the intended honest form for a sampled-city mean; the "~"/"about" is correct precision-discipline, NOT a hedge. Do NOT flag the rounded value as a precision mismatch, and do NOT flag the approximation marker as hedging. A bare 2-decimal citation ("11.53°C") is also acceptable, as is any anomaly cited within ±0.5°C of `headline_metric.value` (`sampled_city_mean_anomaly_c`). A dropped Fahrenheit conversion is fine — an anomaly delta in °F is optional, not required.
