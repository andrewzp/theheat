...
**(HISTORICAL — the standalone claim-extractor module was deleted as dead code in 0.9.20.0 / [#210](https://github.com/andrewzp/theheat/pull/210); the fact-checker extracts claims in-place, see the 2026-05-22 entry above. The `claim_extractor` suppression stage is no longer producible.)** Claim-extractor failure tagging was live 2026-05-17 → 0.9.20.0 (CHANGELOG 0.7.2.0 / #126): pipeline wrapped `claim_extractor.extract_claims()` in try/except and recorded `kill_stage="claim_extractor"` instead of letting the exception bubble to generic `pipeline_error`, with JSON-parse retry parity (mirrors #121 from 0.7.1.0).
...
flowchart TD
    ...
    INTERN --> NEGCACHE{"Cross-cycle negative cache<br/>(economics P1.3)<br/>refill: LAST $0 predicate<br/>legacy: pre-cap partition<br/>min_kills fresh kills on same<br/>(sha, epoch, stage)?"}:::gate
    ...
    WRITER -->|tweet text| FACTCHECK["Gemini Flash<br/>Fact-checker<br/>(90s timeout)<br/>extracts claims in-place (#210)<br/>strict entity match vs bundle"]:::gen
