...
**Last updated:** 2026-06-13 (release 0.9.63.0: the 30-item audit backlog / THIRTY-LOOP is COMPLETE — Codex shipped S-01..S-35, PRs #222–#265; supply flags `WRITER_SAMPLES=2` + `CRITIC_REVISE_ENABLED=1` are LIVE). Authoritative current state: [/Users/andrewpuschel/Documents/Claude/theheat/docs/handoffs/2026-06-13.md](/Users/andrewpuschel/Documents/Claude/theheat/docs/handoffs/2026-06-13.md). The editorial-pipeline shape described below is unchanged — the loop hardened the state/fetch/observability/dashboard layers and added supply levers behind flags. (Earlier: 0.9.22.0 declarative `MERGE_SPEC` — the 314-line `_merge_state` is now a 54-key strategy table + 6-line driver with structural contract `set(MERGE_SPEC) == set(DEFAULT_STATE)`; **new `DEFAULT_STATE` keys REQUIRE a `MERGE_SPEC` entry**; [#215](https://github.com/andrewzp/theheat/pull/215).) Prior: 0.9.21.0 (source-health correctness — `air_quality` recovers Open-Meteo rate-limited cities; the sentinel no longer false-flags a consistently-`degraded` source; #201/#202 closed), 0.9.20.0 (post-Part-B hardening + tech-stack review), gpm feed live on `datapool` (0.9.15.0/0.9.16.0). These harden the state, data-fetch + observability layers; the editorial pipeline shape described below is unchanged.
