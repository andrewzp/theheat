...
**Last updated:** 2026-07-24 (version 0.9.108.0 on main; PRs #464/#463 carry 0.9.109.0/0.9.110.0; changes under CHANGELOG `[Unreleased]`). **Canonical production resume: [/Users/andrewpuschel/Documents/Claude/theheat/docs/handoffs/2026-07-24-next-session.md](/Users/andrewpuschel/Documents/Claude/theheat/docs/handoffs/2026-07-24-next-session.md)** (supersedes all prior handoffs). North star: [docs/VISION.md](/Users/andrewpuschel/Documents/Claude/theheat/docs/VISION.md); prior program (COMPLETE, rows 1–8): [docs/superpowers/plans/2026-07-03-three-pillar-upgrade-plan.md](/Users/andrewpuschel/Documents/Claude/theheat/docs/superpowers/plans/2026-07-03-three-pillar-upgrade-plan.md); current program (**ADOPTED — all executable rows MERGED; Bet A fully LIVE**): [docs/superpowers/plans/2026-07-06-front-page-parity-plan.md](/Users/andrewpuschel/Documents/Claude/theheat/docs/superpowers/plans/2026-07-06-front-page-parity-plan.md) + [docs/superpowers/plans/front-page-parity/INDEX.md](/Users/andrewpuschel/Documents/Claude/theheat/docs/superpowers/plans/front-page-parity/INDEX.md), all on main.

**Status (2026-07-24): THE ECONOMICS ERA — P0+P1 SHIPPED, P2 EXECUTING; BILLING IS THE RECURRING GATE.** The 2026-07-11 billing outage exposed the run-rate (~$50–70/mo, test suite outspending the product 2:1) and produced [PLAN-ECONOMICS-MASTER-v3](/Users/andrewpuschel/Documents/Claude/theheat/docs/superpowers/plans/economics/PLAN-ECONOMICS-MASTER-v3.md) (mandate: Claude stays the writer; cost ≤25%). **P0+P1 shipped 2026-07-14** (#439 cycle billing breaker + one retry owner; #443 voice-regression → writer-path PR gate + daily 09:17Z billing canary + weekly full; #444 self-heal keyless gate + Haiku pin; #445 per-call usage ledger → `state.llm_usage`; #447 budget watch 70%/90% vs `THEHEAT_MONTHLY_BUDGET_USD`; #446 dead legacy pipeline deleted; flags `SAMPLES=1`+`REVISE=0`). Production was **stopped** on Andrew's order mid-train (#441) and **restored 2026-07-16** (#449, + the 12:00-leaderboard mode fix). **Week-1 ledger measured ~$57/mo annualized** (peak-season queues × ~6-attempt cap × ~1.85 retry multiplier); the budget watch fired [#461](https://github.com/andrewzp/theheat/issues/461) on 07-22 and the balance **emptied again 07-24** — Console auto-reload+cap is the standing Andrew item. **P2 in flight:** [#464](https://github.com/andrewzp/theheat/pull/464) cross-cycle negative cache (min-kills=2 + decision epoch) and [#463](https://github.com/andrewzp/theheat/pull/463) writer structured outputs (codex-approved, billing-held on its voice gate); **P2.1 Batch API writer lane is the next build** (50% off, stacks with caching → ~$15–22/mo peak-season). Ops: metrics lane 401s on X reads since 07-07 (#452 — X portal read tier, Andrew). Full state + paste-ready kickoff in the 2026-07-24 handoff.

**Status (2026-07-08 — now historical): ALL EXECUTABLE ROWS MERGED; BET A FULLY LIVE (A0+A1+A2); HEAT-DOME PIVOTED TO GLOBAL (#414).** This session shipped the two editorial follow-ups — **#401** ([#408](https://github.com/andrewzp/theheat/pull/408), precip "300 mm monitoring threshold" cross-gate: aligned writer + fact-check TO the critic) and **#403** ([#409](https://github.com/andrewzp/theheat/pull/409), coral DHW `_stress_level_for_dhw` extended to the full NOAA scale 12→L3/16→L4/20→L5) — plus **row 9** ([#412](https://github.com/andrewzp/theheat/pull/412), engagement-capture: the dormant metrics lane made flippable). **Three prod flags flipped LIVE (Andrew's go):** `THEHEAT_NEWS_BOOST_ENABLED=1` (A2) + `THEHEAT_PER_COUNTRY_CAP=2` (20:42Z), then `THEHEAT_METRICS_ENABLED=1` (row 9, 22:43Z). Boost/cap watch is **regression-clean** across the first post-flip cycles (newsworthiness healthy, P0 guard holds, no boost fired yet — rare/days-out). **Heat-dome arc:** the row-13 US population-extent spike went GO but Andrew flagged **US-only is off-brand** for a global account, so PR-A ([#415](https://github.com/andrewzp/theheat/pull/415)) was **reverted** ([#417](https://github.com/andrewzp/theheat/pull/417)) and the story re-pointed at the **global records-cluster** ([#414](https://github.com/andrewzp/theheat/issues/414), spiked GO [#416](https://github.com/andrewzp/theheat/pull/416)) — cluster record-breaking cities SPATIALLY (cross-border), name honestly via continent + country list. **#414 is the next build** (global, codex-gated). Remaining rows gated: **8** (days of `news_events`), **10** (A2 live ≥1 week ~07-14), **12** (2–4 weeks data). Andrew pending: row-9 Vercel deploy. Full detail + paste-ready #414 kickoff in the 2026-07-08 handoff.
...
anthropic>=0.77       # Claude Sonnet 4.6 — the WRITER (evaluator deleted in #446; floor rides #463's structured outputs)
