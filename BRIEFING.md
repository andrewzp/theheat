...
**Architecture status (post-0.9.8.0):** Pipeline shape unchanged from 0.9.0.0 — `sources (all 23 enqueue into the per-cycle triage queue via _enqueue_story_candidate) → TRIAGE (rank + per-category cap + pending-type cap + global cap) → EVIDENCE CONTRACT → writer (Sonnet 4.6, prompt-cached) → safety → fact_check (Gemini Flash, also extracts claims in-place; unknown kinds are skipped not killed) → critic (Gemini 2.5 Pro; assesses signals relative to available baseline depth) → pending`. New since 0.9.4.0: triage runs `apply_pending_ttl_sweep` at the START of each cycle to auto-reject pending drafts older than `THEHEAT_PENDING_TTL_DAYS` (default 7), then enforces `THEHEAT_PENDING_TYPE_CAP` (default 3) per `legacy_type` to prevent monoculture pile-ups. Observability layer: dashboard reads workflow + routine + posting-mode state every 30s and renders a 4-dot strip. Routine writes its own health beacon to the `ROUTINE_BEACON` repo variable at end of every cycle.

**Production state (at this release):** bot workflows `theheat-bot` and `voice-regression` **re-enabled 2026-06-01** after a 12-day pause. `refresh-thresholds` continues (data-only, no LLM). Multiple natural cron cycles have fired under the 0.9.x architecture; pipeline runs cleanly end-to-end. As of 23:13 UTC 2026-06-01: pending queue 0, latest run `success` with 0 failures; new fact_check kills since 0.9.8.0 = 0 (the parser fix held); new critic kills since 0.9.7.0 cite **legitimate editorial reasons** (`recycled_phrasing`, `template_convergence`, `interesting_but_not_memorable` with new "Wait, what?" reasoning) NOT the broken "26-year too short" pattern. mypy clean across 92 source files; pytest 1363 passing on main; dashboard tests passing.
...
**Posting status:** still **manual_only** for all categories. A-rate signal under the now-fully-unblocked 0.9.x architecture is the lever for posting flip. The 5+ days of zero-draft cycles ended on 2026-06-01 when PR #164 + #165 fixed the funnel-stage bugs; first real A-rate observation cycle starts here.
