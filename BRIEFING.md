...
**Architecture status (post-0.9.4.0):** Pipeline unchanged from 0.9.0.0 — `sources (all 23 enqueue into the per-cycle triage queue via _enqueue_story_candidate) → TRIAGE → EVIDENCE CONTRACT → writer (Sonnet 4.6, prompt-cached) → safety → fact_check (Gemini Flash, also extracts claims in-place) → critic (Gemini 2.5 Pro) → pending`. Observability layer: dashboard reads workflow + routine + posting-mode state every 30s and renders a 4-dot strip. Workflow last-run reads are production-branch scoped, posting-mode read failures surface as unavailable, and the automation route caches briefly server-side. Routine writes its own health beacon to the `ROUTINE_BEACON` repo variable at end of every cycle.

**Production state (at this release):** bot workflows `theheat-bot` and `voice-regression` remain **disabled in CI** pending Andrew's call to re-enable. `refresh-thresholds` continues (data-only, no LLM). The full architecture still has never run end-to-end in production. Four routine fires under the rewritten prompt (5/23 through 5/26) all succeeded; beacon writes started landing reliably with PR #160. mypy clean; pytest passing on main; dashboard tests passing.

**Open PRs:** none. The three previously-parked `wip/` branches were archived 2026-05-26 as tags `archive/wip/fact-check-disposition-tightening`, `archive/wip/climate-indices-cadence`, `archive/wip/era-anchors-safety-curation`. Resurrect with `git checkout -b wip/<name> archive/wip/<name>`.
