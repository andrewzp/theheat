...
**Last updated:** 2026-05-22 late session (release 0.9.1.0 — dashboard automation indicators + routine prompt rewrite). Followed 0.9.0.0 (post-Codex-sweep: evidence contract + all-sources triage + fact-check/claim-extract collapse + sea_ice & ice_mass URL fixes + Earthdata operator actions completed).
**Status:** **DATA LAYER COMPLETE; 23 SOURCES LIVE; EDITORIAL PIPELINE END-TO-END HARDENED; ALL SOURCES ON THE TRIAGE PATH; EVIDENCE CONTRACT GATES THE WRITER; DASHBOARD AUTOMATION INDICATORS LIVE; ROUTINE STALE-SNAPSHOT BUG FIXED; POSTING STILL PAUSED PENDING ANDREW'S RE-ENABLE SIGNAL ON NEW ARCHITECTURE.** The 0.7.1.0 release added the editorial loop (F3 critic). The 0.7.2.0 release added post-wave hardening + the triage spec. The 0.8.0.0 release shipped the triage MVP + Anthropic prompt caching. The 0.9.0.0 release completed the triage migration + evidence contract. **The 0.9.1.0 release (this one) ships the dashboard's persistent "Automation" status strip (4 colored dots + posting-mode pill, read-only) + a routine prompt rewrite that fixes the stale-snapshot bug (PR #152's confused re-grade) AND writes a routine_last_run_at health beacon to the gist on every cycle.** See [/Users/andrewpuschel/Documents/Claude/theheat/CHANGELOG.md](/Users/andrewpuschel/Documents/Claude/theheat/CHANGELOG.md) 0.9.1.0 for the full release notes and [/Users/andrewpuschel/Documents/Claude/theheat/docs/handoffs/2026-05-22-v2.md](/Users/andrewpuschel/Documents/Claude/theheat/docs/handoffs/2026-05-22-v2.md) for the current next-session prompt.
