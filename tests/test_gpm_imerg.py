...
class TestPrecipDetection:
    ...
    def test_multi_day_accumulation_is_a_threshold_not_a_record(self):
        ...
        for event in rolling:
            assert event.previous_record_mm is None
            assert event.previous_record_year is None
            assert event.deviation_from_record_mm is None
            assert event.alert_threshold_mm == 150.0  # the 3-day trigger
