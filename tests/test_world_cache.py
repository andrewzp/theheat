...
def attributed(cache, country="Spain", lat=40.4, lon=-3.7):
    return {K(city, country, lat, lon): {**row, "identity": sample_identity(city, country, lat, lon)} for city, row in cache.items()}
...
def test_merge_equal_as_of_is_field_wise_more_extreme():
    ...
    assert out[K("Madrid")]["all_time_max"] == [45.5, 2026]   # hotter high wins
    assert out[K("Madrid")]["monthly_min"]["06"] == [8.0, 1997]  # colder low wins
...
def test_provisional_suppresses_all_time_and_monthly_re_fire():
    base = CityThresholds(identity=sample_identity("Madrid", "Spain", 40.4, -3.7), city="Madrid", as_of="2026-06-01", years_of_data=30,
        all_time_max=(44.0, 2023), monthly_max={"06": (43.0, 2019)}).to_dict()
    cache = {K("Madrid"): base}   # world cache is keyed by world_key (city|country)
    fc = {"max_c": 45.5, "min_c": 20.0, "tw_max_c": 10.0}
    b1 = evaluate_city("Madrid", "Spain", fc, CityThresholds.from_dict(cache[K("Madrid")]), lat=40.4, lon=-3.7, today=_date(2026, 6, 26))
    assert b1.all_time_high and b1.monthly_high
    apply_provisional(cache, b1, today="2026-06-26")
    b2 = evaluate_city("Madrid", "Spain", fc, CityThresholds.from_dict(cache[K("Madrid")]), lat=40.4, lon=-3.7, today=_date(2026, 6, 27))
    assert b2.all_time_high is None and b2.monthly_high is None
    assert cache[K("Madrid")]["all_time_max"][0] == 45.5 and cache[K("Madrid")]["monthly_max"]["06"][0] == 45.5
...
def test_apply_provisional_preserving_as_of_branches():
    ...
    def base(as_of):
        return CityThresholds(identity=sample_identity("C", "Spain", 40, -3), city="C", as_of=as_of, years_of_data=30,
    ...
    ck = K("C", "Spain", 40, -3)   # world cache key = world_key(bundle.city, bundle.country)
