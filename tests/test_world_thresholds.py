...
def test_compute_means_use_independent_high_low_counts():
    ...
    t = compute_city_thresholds("T", archive, as_of="2026-06-26", years_of_data=31)
    mh, ml, n, low_n = t.monthly_mean["06"]
    assert mh == 41.0
...
def _cached(mean_n=900):
    return CityThresholds.from_dict(snapshot(CityThresholds(identity=sample_identity("Madrid", "Spain", 40.4, -3.7), city="Madrid", as_of="2026-06-26", years_of_data=30,
        all_time_max=(44.0, 2023), all_time_min=(-4.0, 2001),
        monthly_max={"06": (43.0, 2019)}, monthly_min={"06": (8.0, 1997)},
        monthly_mean={"06": (32.0, 17.0, mean_n)}, wetbulb_max=(26.0, 2022)).to_dict()))


def test_evaluate_emits_all_time_high_no_calendar():
    b = evaluate_city("Madrid", "Spain", dated_forecast({"max_c": 45.5, "min_c": 22.0, "tw_max_c": 24.0}),
...
def test_evaluate_skips_anomaly_when_mean_is_sparse():
    hot = dated_forecast({"max_c": 48.0, "min_c": 20.0, "tw_max_c": 10.0})
