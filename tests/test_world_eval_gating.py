"""Tests for the world-half eval/warm/consolidate fix (_run_world_cached_half).

Covers the eval-gating defect (archive saturation must NOT starve forecast
evaluation), the refute-before-emit guarantee (a fresh archive refutes a
stale-cache record before it is emitted or stamped), and freshness honesty
(a record fired against still-stale climatology keeps a stale as_of so the
city stays warm-eligible).
"""
...
def _run(monkeypatch, world_cities, seed_cache, *, forecasts, archive,
    ...
    """Run _run_world_cached_half with patched IO.

    forecasts: callable(cities)->{city: fc} (or raises). archive: callable(city)->archive
    (or raises/None). Returns (om_bundles, om_country, store, metrics).
    """
...
def _fc_map(mapping):
    def f(cities):
        return {c["city"]: mapping[c["city"]] for c in cities if c["city"] in mapping}
    return f
...
def test_eval_runs_despite_warm_archive_saturation(monkeypatch):
    seed = {
        "C1": _thresh("C1", ISO, all_time_max=(40.0, 2000)),
        "C2": _thresh("C2", ISO, all_time_max=(40.0, 2000)),
    }
    # Aaa_ok = missing warm target that SUCCEEDS (cache grows -> growing bootstrap);
    # Zzz_429 = missing warm target that 429s (sets warm_saturated). Name order makes
    # select_stale_cities warm Aaa_ok before Zzz_429.
    cities = [_city("C1"), _city("C2"), _city("Aaa_ok"), _city("Zzz_429")]
    fc_calls = {"n": 0}

    def forecasts(batch):
        fc_calls["n"] += 1
        return {c["city"]: {"max_c": 45.0, "min_c": 12.0, "tw_max_c": 10.0} for c in batch}
    ...
    om_bundles, _om_country, store, m = _run(
        ...
        forecasts=forecasts, archive=archive,
    )
    assert m["forecast_attempted"] == 2           # both cached cities evaluated despite the archive 429
    assert m["coverage_ratio"] == round(2 / 4, 3)  # denominator is full world count
    assert m["eval_saturated"] is False
    assert m["warm_saturated"] is True            # Zzz_429 tripped the archive limit
    assert m["saturated"] is True                 # backward-compatible OR
    assert m["cached_count"] == 3                  # C1, C2 + warmed Aaa_ok
    assert m["status"] != "degraded"              # decoupled: warm 429 during healthy growth is benign
    assert {b.city for b in om_bundles} == {"C1", "C2"}   # Aaa_ok warmed but not eval'd (1-run lag)
    assert all(b.all_time_high is not None for b in om_bundles)
    assert fc_calls["n"] == 1                      # single batched forecast fetch; CONSOLIDATE is network-free


# --- 2. Eval saturation degrades
def test_eval_saturation_degrades(monkeypatch):
    seed = {"C1": _thresh("C1", ISO, all_time_max=(40.0, 2000))}
    cities = [_city("C1")]
...
def test_stale_record_confirmed_by_fresh_archive(monkeypatch):
    seed = {"C": _thresh("C", STALE_ISO, all_time_max=(44.0, 2000))}
    cities = [_city("C")]
    ...
    assert any(b.all_time_high is not None for b in om_bundles)   # 47.0 > fresh 46.0
    assert store["C"]["all_time_max"] == [47.0, YEAR]
    assert store["C"]["as_of"] == ISO                            # warmed -> as_of advances
...
def test_stale_record_refuted_by_fresh_archive(monkeypatch):
    seed = {
        "Hi": _thresh("Hi", STALE_ISO, all_time_max=(44.0, 2000)),
        "Lo": _thresh("Lo", STALE_ISO, all_time_min=(-12.0, 2000)),
    }
    ...
    om_bundles, _c, store, _m = _run(
        monkeypatch, cities, seed,
        forecasts=_fc_map({
            "Hi": {"max_c": 45.5, "min_c": 0.0, "tw_max_c": 10.0},   # would beat stale 44, not fresh 46
            "Lo": {"max_c": 5.0, "min_c": -14.0, "tw_max_c": 10.0},  # would beat stale -12, not fresh -16
        }),
        archive=archive,
    )
    ...
    assert store["Hi"]["all_time_max"] == [46.0, ARCH_YEAR]   # fresh archive value, NOT 45.5
    assert store["Lo"]["all_time_min"] == [-16.0, ARCH_YEAR]  # fresh archive value, NOT -14.0


# --- 5a. No stale-field carry-over through the stamp (warm succeeds -> fresh climatology wins)
def test_no_stale_field_carryover_warm_success(monkeypatch):
    ...
    assert store["C"]["all_time_max"] == [47.0, YEAR]            # today's record stamped
    assert store["C"]["monthly_mean"][MM] == [46.0, 12.0, 1]     # FRESH archive, not stale (20,10,50)
    assert store["C"]["wetbulb_max"] == [28.0, ARCH_YEAR]        # FRESH archive, not stale (30,2005)
...
def test_no_stale_field_carryover_warm_fails(monkeypatch):
    ...
    assert store["C"]["all_time_max"] == [47.0, YEAR]            # provisional stamped vs still-stale base
    assert store["C"]["monthly_mean"][MM] == [20.0, 10.0, 50]    # untouched stale
    assert store["C"]["wetbulb_max"] == [30.0, 2005]            # untouched stale
    assert store["C"]["as_of"] == STALE_ISO                      # archive unconfirmed -> as_of preserved
    assert any(b.all_time_high is not None for b in om_bundles)
    assert m["warm_saturated"] is True


# --- 6. Freshness honesty: stale record-bearing city NOT warmed keeps stale as_of (still warm-eligible)
def test_freshness_honesty_not_warmed_keeps_stale_as_of(monkeypatch):
    seed = {
        "Aaa": _thresh("Aaa", STALE_ISO, all_time_max=(44.0, 2000)),
        "Bbb": _thresh("Bbb", STALE_ISO, all_time_max=(44.0, 2000)),
    }
    cities = [_city("Aaa"), _city("Bbb")]

    def archive(c):
        if c["city"] == "Bbb":
            raise OpenMeteoSaturated("archive 429")   # Bbb never gets a fresh archive
        return _arch(max_c=46.0)
    ...
    assert store["Aaa"]["as_of"] == ISO                          # warmed -> fresh
    assert store["Bbb"]["as_of"] == STALE_ISO                    # not warmed -> stays stale-eligible
    assert world_cache._is_stale(store["Bbb"], ttl_days=30, today=ISO) is True
...
def test_snapshot_before_warm_one_run_lag(monkeypatch):
    seed = {"C": _thresh("C", ISO, all_time_max=(40.0, 2000))}
    ...
    assert m["forecast_attempted"] == 1                  # only C eval'd (M missing pre-warm)
    assert "M" in store                                  # M warmed this run
    assert m["coverage_ratio"] == round(1 / 2, 3)
    assert {b.city for b in om_bundles} == {"C"}         # no M bundle this run (1-run lag)


# --- 10. Steady-state cached record path: as_of advances to today (dominant production path)
def test_steady_state_cached_record_advances_as_of(monkeypatch):
    seed = {"C": _thresh("C", ISO, all_time_max=(40.0, 2000))}   # fresh -> not stale, not warmed
    cities = [_city("C")]
    om_bundles, _c, store, _m = _run(
        monkeypatch, cities, seed,
    ...
    assert store["C"]["all_time_max"] == [45.0, YEAR]
    assert store["C"]["as_of"] == ISO                    # fresh+not-warmed record -> as_of advances
    assert any(b.all_time_high is not None for b in om_bundles)
...
def test_cold_cache_bootstrap_success(monkeypatch):
    ...
    _b, _c, store, m = _run(
        monkeypatch, cities, {},
        forecasts=_fc_map({}),                            # nothing cached to eval
        archive=lambda c: _arch(max_c=46.0),              # warm both missing cities
    )
    assert m["forecast_attempted"] == 0
    assert m["coverage_ratio"] == 0.0
    assert m["cached_count"] == 2                         # both warmed
    assert m["status"] == "success"                       # bootstrap grew the cache
...
def test_non_record_signal_survives_no_as_of_advance(monkeypatch):
    seed = {"C": _thresh("C", STALE_ISO, all_time_max=(50.0, 2000))}   # forecast won't beat it
    cities = [_city("C")]
    ...
    om_bundles, _c, store, _m = _run(
        monkeypatch, cities, seed,
        forecasts=_fc_map({"C": {"max_c": 45.0, "min_c": 12.0, "tw_max_c": 10.0}}),
        archive=_raise(OpenMeteoSaturated("archive 429")),    # C stays not-warmed
        abs_extreme=lambda *a, **k: ev,
    )
    assert any(b.absolute_extreme is not None for b in om_bundles)
    assert not any(b.all_time_high is not None for b in om_bundles)    # no record
    assert store["C"]["as_of"] == STALE_ISO                           # no stamp -> as_of untouched
    assert store["C"]["all_time_max"] == [50.0, 2000]                 # unchanged
...
def test_country_record_refuted_then_confirmed_over_post_warm(monkeypatch):
    # Two stale-and-cached Spain cities, both understated (cached all_time_max 44.0) so the
    # forecast would fire a city record vs the STALE cache. detect_country_records runs for
    # real over the post-warm re-derived all_readings.
    seed = {"A": _thresh("A", STALE_ISO, all_time_max=(44.0, 2000)),
            "B": _thresh("B", STALE_ISO, all_time_max=(44.0, 2000))}
    cities = [_city("A", country="Spain"), _city("B", country="Spain")]

    # Refuted: fresh archives (46.0) are HOTTER than today (45.5) -> no country high.
    ...
    # Confirmed: today (47.0) still beats the fresh archive (46.0) -> country high fires.
